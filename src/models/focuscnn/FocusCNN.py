...
class FocusCNN(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        Returns:
            str: model information as string
        """
        focus_details = "\n\n".join([f"Focus({cls_idx}):\n{str(model)}" for cls_idx, model in self.focus_models.items()])
        return "Trainable parameters:\nFocus network params:{}\n\nClassifier" " params:{}".format(
            focus_details, str(self.classifier_model)
        )
    ...
    def load_model(
        ...
        assert classifier_model_path is not None and all(
            [focus_model_path is not None for focus_model_path in focus_models_path.values()]
        ), "Models checkpoint paths are not specified!"

        assert set(focus_models_path.keys()) == set(self.focus_models.keys()), "Focus models ids are not matching!"

        self.classifier_model.load_state_dict(torch.load(classifier_model_path)["state_dict"])
    ...
    def forward(self, x, target):
        outs_focus = {cls_idx: focus_model(x, target[cls_idx]) for cls_idx, focus_model in self.focus_models.items()}
        ...
        inputs_cls = torch.cat([self.tf_module(x, out_focus_cat) for out_focus_cat in outs_focus_cat.values()])
        target_cls = torch.cat([target[cls_idx]["label"] for cls_idx in self.focus_models.keys()])
        ...
        return {
            "out_cls": outs_cls,
            "outs_translate_x": {cls_idx: out_focus["out_translate_x"] for cls_idx, out_focus in outs_focus.items()},
            "outs_translate_y": {cls_idx: out_focus["out_translate_y"] for cls_idx, out_focus in outs_focus.items()},
            "outs_scale": {cls_idx: out_focus["out_scale"] for cls_idx, out_focus in outs_focus.items()},
    ...
    def get_predictions(self, output, img_ids):
        ...
        for img_idx in range(img_ids.shape[0]):
            ...
            for cls_idx in self.focus_models.keys():
                if predictions[(int(cls_idx) - 1) * img_ids.shape[0] + img_idx] == int(cls_idx):
                    # prediction for own class
                    n_predictions += 1
                    image_predictions["scores"].append(float(confidences[(int(cls_idx) - 1) * img_ids.shape[0] + img_idx]))
            ...
            image_predictions["boxes"] = torch.Tensor(image_predictions["boxes"])
            image_predictions["scores"] = torch.Tensor(image_predictions["scores"])
            image_predictions["labels"] = torch.LongTensor(image_predictions["labels"])
    ...
    def prepare_target_for_map(self, target_bboxes, target_image_ids):
        ...
        for image_id in target_image_ids:
            image_bboxes = target_bboxes[target_bboxes[:, 0] == image_id]  # Filter bboxes for current image
